...
BFE模块代表一个高内聚、低耦合、实现某种流量处理功能（例如检测、过滤、改写等）的代码单元。在BFE源码的 bfe_modules/<module_name> 目录中内置了大量的扩展模块。
...
模块除了需要实现以上基础接口外，往往还需实现如下的特定回调接口。
...
| 回调点               | 回调点含义         | 回调函数类型   |
| -------------------- | ------------------ | -------------- |
| HandleAccept         | 连接建立成功后     | AcceptFilter   |
| HandleHandshake      | 连接TLS握手完成后  | AcceptFilter   |
...
| HandleReadResponse   | 读取到响应头部时   | ResponseFilter |
| HandleRequestFinish  | 响应发送完成时     | ResponseFilter |
| HandleFinish         | 连接处理完成时     | FinishFilter   |

我们可以访问BFE实例的 http://localhost:8299/monitor/module_handlers 查看运行的BFE实例中在各回调点注册的模块回调函数。
